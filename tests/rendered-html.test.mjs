...
test("server-renders the Prompt Studio mission", async () => {
  ...
  assert.match(html, /Texte &amp; sources/);
  assert.match(html, /Challenge métier/);
  assert.match(html, /Expériences facultatives/);
...
test("ships the finished interface and its workshop resources", async () => {
  ...
  assert.match(page, /LAB IMAGE · INTENTION VISUELLE/);
  assert.match(page, /PRÉCISION UTILE/);
  assert.match(page, /Prompt Battle/);
  assert.match(page, /Prompt Repair/);
  assert.match(page, /La consigne inutile/);
  assert.match(page, /Image Challenge/);
  assert.match(page, /Pour aller plus loin/);
  assert.match(page, /MON BESOIN PROFESSIONNEL/);
  assert.match(page, /Et si je devais refaire cette tâche/);
  assert.doesNotMatch(page, /Bibliothèque personnelle/i);
  assert.doesNotMatch(page, /Variables rapides/i);
  assert.doesNotMatch(page, /PROMPT PROPRE À TESTER/);
  assert.doesNotMatch(page, /TEST RÉEL · 10 MIN/);
